#pragma once
#include<stdio.h>
#include<glew.h>
#include<glfw3.h>

class Window
{
public:
	Window();
	Window(GLint windowWidth, GLint windowHeight);
	int Initialise();
	GLfloat getBufferWidth() { return bufferWidth; }
	GLfloat getBufferHeight() { return bufferHeight; }
	bool getShouldClose() { return glfwWindowShouldClose(mainWindow); }
	bool* getsKeys() { return keys; }
	GLfloat getXChange();
	GLfloat getYChange();
	void swapBuffers() { return glfwSwapBuffers(mainWindow); }

	// artuculaciones gato
	GLfloat getCuello() { return cuello; }
	GLfloat getCola() { return cola; }
	GLfloat getPataA(int index) { return pataA[index]; } 
	GLfloat getPataB(int index) { return pataB[index]; } 

	~Window();
private:
	GLFWwindow* mainWindow;
	GLint width, height;
	bool keys[1024];
	GLint bufferWidth, bufferHeight;
	GLfloat lastX, lastY, xChange, yChange;
	bool mouseFirstMoved;

	// variables gatito
	GLfloat cuello, cola;
	GLfloat pataA[4]; 
	GLfloat pataB[4]; 

	void createCallbacks();
	static void ManejaTeclado(GLFWwindow* window, int key, int code, int action, int mode);
	static void ManejaMouse(GLFWwindow* window, double xPos, double yPos);
};

//#pragma once
//#include<stdio.h>
//#include<glew.h>
//#include<glfw3.h>
//
//class Window
//{
//public:
//	Window();
//	Window(GLint windowWidth, GLint windowHeight);
//	int Initialise();
//	GLfloat getBufferWidth() { return bufferWidth; }
//	GLfloat getBufferHeight() { return bufferHeight; }
//	bool getShouldClose() {
//		return  glfwWindowShouldClose(mainWindow);}
//	bool* getsKeys() { return keys; }
//	GLfloat getXChange();
//	GLfloat getYChange();
//	void swapBuffers() { return glfwSwapBuffers(mainWindow); }
//	GLfloat getrotay() { return rotay; }
//	GLfloat getrotax() { return rotax; }
//	GLfloat getrotaz() { return rotaz; }
//	GLfloat getarticulacion1() { return articulacion1; }
//	GLfloat getarticulacion2() { return articulacion2; }
//	GLfloat getarticulacion3() { return articulacion3; }
//	GLfloat getarticulacion4() { return articulacion4; }
//	GLfloat getarticulacion5() { return articulacion5; }
//	GLfloat getarticulacion6() { return articulacion6; }
//	// Agregamos nuevas para los movimientos de las llantas
//	GLfloat getLlanta1() { return llanta1; }
//	GLfloat getLlanta2() { return llanta2; }
//	GLfloat getLlanta3() { return llanta3; }
//	GLfloat getLlanta4() { return llanta4; }
//
//	
//
//	~Window();
//private: 
//	GLFWwindow *mainWindow;
//	GLint width, height;
//	GLfloat rotax,rotay,rotaz, articulacion1, articulacion2, articulacion3, articulacion4, articulacion5, articulacion6;
//	//De igual forma aqui
//	GLfloat llanta1, llanta2, llanta3, llanta4;
//	bool keys[1024];
//	GLint bufferWidth, bufferHeight;
//	GLfloat lastX;
//	GLfloat lastY;
//	GLfloat xChange;
//	GLfloat yChange;
//	bool mouseFirstMoved;
//	void createCallbacks();
//	static void ManejaTeclado(GLFWwindow* window, int key, int code, int action, int mode);
//	static void ManejaMouse(GLFWwindow* window, double xPos, double yPos);
//};
//

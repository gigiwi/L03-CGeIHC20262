...
int Window::Initialise()
{
	...
	mainWindow = glfwCreateWindow(width, height, "Practica 5: Coche Jerarquico", NULL, NULL);
	if (!mainWindow)
	{
...
void Window::ManejaTeclado(GLFWwindow* window, int key, int code, int action, int mode)
{
	...
	if (action == GLFW_PRESS || action == GLFW_REPEAT)
	{
		...
		if (key == GLFW_KEY_E)
		{
			...
			if (theWindow->anguloCofre > 45.0f) { theWindow->anguloCofre = 45.0f; } //limitando a 45 ára abrir
		}
		if (key == GLFW_KEY_R)
		{
			theWindow->anguloCofre -= 2.0f;
